...
with selectProp:
    propList = list(matk_prettyNamesDict.keys())
    selectProp = st.multiselect('Attributes to vary',propList,
                                format_func=lambda x : matk_prettyNamesDict[x])
with valueSlider:
...
with flatBonusMatk:
    flatBonusMatk = numInput('Flat Matk bonus',step=1,key='flatBonusMatk',
            value=defaultDict['flatBonusMatk'])
with race:
...
if st.session_state.calculate:
    matk = Matk(level,aint,adex,aluk,weapon,flatBonusMatk,race,mproperty,size,
